	...
	public boolean parseCommunityBoardCommand(String command, Player player)
	{
		...
		if (command.equals("_bbshome") || command.equals("_bbstop"))
		{
			...
			if (!Config.CUSTOM_CB_ENABLED)
			{
				...
				returnHtml = returnHtml.replace("%online_players%", String.valueOf(World.getInstance().getPlayers().size()));
				//returnHtml = returnHtml.replace("%expPlayer%", String.valueOf(player.getExp()));
				//вывод рейтов сервера для кб
				returnHtml = returnHtml.replace("%exp%", String.valueOf(Config.RATE_XP));
				returnHtml = returnHtml.replace("%sp%", String.valueOf(Config.RATE_SP));
				returnHtml = returnHtml.replace("%quest%", String.valueOf(Config.RATE_QUEST_REWARD));
				returnHtml = returnHtml.replace("%drop%", String.valueOf(Config.RATE_DEATH_DROP_AMOUNT_MULTIPLIER));
				returnHtml = returnHtml.replace("%spoil%", String.valueOf(Config.RATE_SPOIL_DROP_AMOUNT_MULTIPLIER));
				//вывод информации о персонаже для кб
				returnHtml = returnHtml.replace("%name%", String.valueOf(player.getName()));
				returnHtml = returnHtml.replace("%level%", String.valueOf(player.getLevel()));
				returnHtml = returnHtml.replace("%ip%", String.valueOf(player.getIPAddress()));

				//проверка статуса на дворянство
				if (player.isNoble()) {
					returnHtml = returnHtml.replace("%newbie%", String.valueOf(String.format("Барон")));
				} else {
					returnHtml = returnHtml.replace("%newbie%", String.valueOf(String.format("Кочевник")));
				}
				int pAttack=(int) player.getPAtk(player);
				returnHtml = returnHtml.replace("%pAtack%", String.valueOf(pAttack));

				if (command.startsWith("_test")) {
					final String page = command.replace("_test;", "");
